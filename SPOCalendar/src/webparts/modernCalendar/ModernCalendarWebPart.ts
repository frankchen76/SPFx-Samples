          ...
          this._getListTitles(this.properties.site)
            .then((response2) => {
              ...
              .then((response3) => {
                ...
                for (var _key1 in response3.value) {
                    col.push({key: response3.value[_key1]['InternalName'], text: response3.value[_key1]['Title']});
                }
  ...
  protected onPropertyPaneFieldChanged(propertyPath: string, oldValue: any, newValue: any): void {
      ...
      if ((propertyPath === 'site' || propertyPath === 'siteOther') && newValue) {
        ...
        if (this.properties.other) { siteUrl = this.properties.siteOther; }                 
          this._getListColumns(newValue,this.context.pageContext.web.absoluteUrl)
            .then((response) => {
              var col: IPropertyPaneDropdownOption[] = [];
              for (var _key in response.value) {
        ...
        .then((response1) => {
          ...
          this._getListTitles(siteUrl)
            ...
            if (this.properties.listTitle) {
                ...
                .then((response3) => {
                  var col: IPropertyPaneDropdownOption[] = [];
                  for (var _key2 in response3.value) {
                      col.push({key: response3.value[_key2]['InternalName'], text: response3.value[_key2]['Title']});
                  }
                  this._columnOptions = col;
                  this.colsDisabled = false;
                  this.listDisabled = false;
